...
public abstract class RetryableCallback<T> implements Callback<T> {
    ...
    public void onResponse(Call<T> call, Response<T> response) {
        if (!ApiHelper.isCallSuccess(response))
            if (retryCount++ < TOTAL_RETRIES) {
                ...
                GlobalUtil.printLog(TAG,"Response="+response.body()+" = Message="+response.message()+" =ErrorBody="+response.errorBody());
                retry();
            } else
    ...
    private void retry() {
        GlobalUtil.printLog("SubmitSurveyBgService", "Retry APi call called = ");
