...
public class LevelPainter implements Runnable {
    ...
    public LevelPainter(SurfaceHolder surfaceHolder, Context context, 
    		...
    		double viscosity, boolean lockEnabled,
    		boolean ecoMode) {
        ...
        // vitesse de la bulle
        this.viscosityConstant = viscosity;
	...
	private void setOrientation(Orientation newOrientation) {
		if (!(lockEnabled && locked) || !initialized) {
	        synchronized (this.surfaceHolder) {
	        	...
	        	viscosityValue = levelWidth * viscosityConstant;
	...
	public void onTouch(int touchX, int touchY) {
		if (lockEnabled) {
			if (((orientation == Orientation.TOP || orientation == Orientation.LANDING) 
				...
				locked = !locked;
				BubbleFragment.provider.setLocked(locked);
			}
		}
	}
