package com.connectivityapps.bubble;
...
public final class OrientationProvider implements SensorEventListener {
    ...
    private OrientationProvider() {
		this.displayOrientation = BubbleFragment.context.getWindowManager().getDefaultDisplay().getRotation();
	}
    ...
    public boolean isSupported() {
        if (supported == null) {
            if (BubbleFragment.context != null) {
                sensorManager = (SensorManager) BubbleFragment.context.getSystemService(Context.SENSOR_SERVICE);
    ...
    public void startListening(OrientationListener orientationListener) {
    	final Activity context = BubbleFragment.context;
    	// load calibration
    	calibrating = false;
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (calibrating) {
			calibrating = false;
			Editor editor =BubbleFragment.context.getPreferences(Context.MODE_PRIVATE).edit();
	...
	public final void resetCalibration() {
		...
		try {
			success = BubbleFragment.context.getPreferences(
