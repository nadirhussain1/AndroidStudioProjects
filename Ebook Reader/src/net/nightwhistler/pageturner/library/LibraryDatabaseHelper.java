...
public class LibraryDatabaseHelper extends SQLiteOpenHelper {
	...
	public enum Field { 
		...
		description("text"), cover_image("blob"), progress("integer"), sample_limit("integer"), store_product_id("text"), purchased("integer");
	...
	private static final String DB_NAME = "PatagoniaEbooksLibrary";
    ...
    private static LibraryDatabaseHelper libraryDatabaseHelper=null;
	public static LibraryDatabaseHelper getInstance(Context context) {
		synchronized (LibraryDatabaseHelper.class) {
			if (libraryDatabaseHelper== null) {
				libraryDatabaseHelper = new LibraryDatabaseHelper(context);
			}
		}
		return libraryDatabaseHelper;
	}
	...
	public void updatePurchased( String fileName, int purchased ) {
		...
		ContentValues content = new ContentValues();
		content.put( Field.purchased.toString(), purchased );
	...
	public void updateFilename( String fileNameOld, String fileNameNew ) {
		...
		//EASY CAKE!
		String whereClause = Field.file_name.toString() + " like ?";
		String[] args = { "%" + fileNameOld };

		ContentValues content = new ContentValues();
		content.put( Field.file_name.toString(), fileNameNew );
	...
	public void updatePurchasedBySku( String sku, int purchased ) {
		...
		//EASY CAKE!
		String whereClause = Field.store_product_id.toString() + " like ?";
		String[] args = { "%" + sku };

		ContentValues content = new ContentValues();
		content.put( Field.purchased.toString(), purchased );

		Log.d("SKU", "SKU " + sku + ", PURCHASED: " + purchased + ", ARGS: " + args);
	...
	public void storeNewBook(String fileName, String authorFirstName,
			...
			byte[] coverImage, boolean setLastRead, int sampleLimit, int purchased) {

		Log.d("LibraryDatabase", "Inside storeNewBook LibraryDatabase");
		ContentValues content = new ContentValues();
		...
		content.put(Field.sample_limit.toString(), sampleLimit);
		content.put(Field.purchased.toString(), purchased);
		...
		long row =getDataBase().insert(LIB_BOOKS_TABLE, null, content);
		Log.d("LibraryDatabase", "Book inserted Id="+row+" title="+title);
	}

	public void storeEmptyNewBook( String authorFirstName,
							 String authorLastName, String title, String description,
							 byte[] coverImage, boolean setLastRead, int sampleLimit, String storeProductID, int purchased) {
	...
	private static LibraryBook doConvertRow(Cursor cursor) {
		...
		newBook.setSampleLimit(cursor.getInt(Field.sample_limit.ordinal()));

		newBook.setPurchased(cursor.getInt(Field.purchased.ordinal()));
